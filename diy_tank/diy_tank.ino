...
void loop() {
  if (type != 2)
  {
    ...
    if (ps2x.Button(PSB_PAD_RIGHT)) {
      ...
      //炮台右转
      //      initialAngle <= 5 ? 5 : (initialAngle -= 5);
      //      barrel.write(initialAngle);
    }
    if (ps2x.Button(PSB_PAD_LEFT)) {
      Serial.print("LEFT held this hard: ");
      //炮台左转
      //      initialAngle >= 175 ? 175 : (initialAngle += 5);
      //      barrel.write(initialAngle);
    }
...
void retreat(int speed) {
  //Motor_A
  analogWrite(MotorA_IN1, map(speed, 128, 255, 80, 255));
  digitalWrite(MotorA_IN2, HIGH);
  //Motor_B
  digitalWrite(MotorB_IN1, HIGH);
  analogWrite(MotorB_IN2, map(speed, 128, 255, 80, 255 ));
}
